// Aborted implementation of a mapreduce function; a complete
// implementation is now in cxx_foldable.hh and is called foldMap.

#ifndef CXX_MAPREDUCE_HH
#define CXX_MAPREDUCE_HH
...
typename std::enable_if<
    ...
    R>::type
mapreduce(const F &f, const Op &op, const R &z, const std::array<T, N> &xs) {
  std::size_t s = xs.size();
  R r(z);
  for (std::size_t i = 0; i < s; ++i)
    r = cxx::invoke(op, std::move(r), cxx::invoke(f, xs[i]));
  return r;
}

template <typename F1, typename F2, typename R, typename T, std::size_t N,
          typename T2, typename CT = std::array<T, N>,
          template <typename> class C = kinds<CT>::template constructor>
typename std::enable_if<
    std::is_same<typename cxx::invoke_of<F, R, T, T2, As...>::type, R>::value,
    R>::type
mapreduce2(const F &f, const R &z, const std::array<T, N> &xs,
