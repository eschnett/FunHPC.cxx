...
#if defined BLAS
extern "C" {
void daxpy_(const int &N, const double &alpha, const double *__restrict__ x,
            const int &incx, double *__restrict__ y, const int &incy);
void dcopy_(const int &N, const double *__restrict__ x, const int &incx,
            double *__restrict__ y, const int &incy);
void dgemm_(const char &transa, const char &transb, const int &M, const int &N,
            const int &K, const double &alpha, const double *__restrict__ a,
            const int &lda, const double *__restrict__ b, const int &ldb,
            const double &beta, double *__restrict__ c, const int &ldc,
...
void dgemv_(const char &trans, const int &M, const int &N, const double &alpha,
            ...
            const double *__restrict__ x, const int &incx, const double &beta,
            double *__restrict__ y, const int &incy, int trans_length);
double dnrm2_(const int &N, const double *__restrict__ x, const int &incx);
void dscal_(const int &N, const double &alpha, double *__restrict__ x,
            const int &incx);
}
#endif
...
void gemv(bool trans, double alpha, const matrix_t &a, const vector_t &x,
  ...
  cblas_dgemv(CblasColMajor, trans ? CblasTrans : CblasNoTrans, y.N, x.N, alpha,
              &a(0, 0), a.NI, &x(0), 1, beta, &y(0), 1);
#elif defined BLAS
  if (alpha != 0.0) {
    if (!trans) {
      RPC_ASSERT(a.NJ == x.N);
  ...
  dgemv_(trans ? 'T' : 'N', y.N, x.N, alpha, &a(0, 0), a.NI, &x(0), 1, beta,
         &y(0), 1, 1);
#else
  scal(beta, y);
  if (alpha == 0.0)
...
void gemm(bool transa, bool transb, double alpha, const matrix_t &a,
  ...
  cblas_dgemm(CblasColMajor, transa ? CblasTrans : CblasNoTrans,
              transb ? CblasTrans : CblasNoTrans, c.NI, c.NJ, a.NJ, alpha,
              &a(0, 0), a.NI, &b(0, 0), b.NI, beta, &c(0, 0), c.NI);
#elif defined BLAS
  if (alpha != 0.0) {
    if (!transb) {
      if (!transa) {
  ...
  dgemm_(transa ? 'T' : 'N', transb ? 'T' : 'N', c.NI, c.NJ, a.NJ, alpha,
         &a(0, 0), a.NI, &b(0, 0), b.NI, beta, &c(0, 0), c.NI, 1, 1);
#else
  if (alpha == 0.0) {
    scal(beta, c);
    return;
