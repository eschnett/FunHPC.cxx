...
RPC_CLASS_EXPORT(busywait_action::evaluate);
RPC_CLASS_EXPORT(busywait_action::finish);
...
void run_sync(double time, ptrdiff_t njobs, const vector<int> &locs,
              ptrdiff_t nthreads) {
...
RPC_CLASS_EXPORT(run_single_action::evaluate);
RPC_CLASS_EXPORT(run_single_action::finish);
...
void run_multi(double time, ptrdiff_t njobs, const vector<int> &locs,
               ptrdiff_t nthreads) {
  cout << "    Configuration: action multi...        " << flush;
  RPC_ASSERT(nthreads ==
             ptrdiff_t(locs.size() * thread::hardware_concurrency()));
  vector<future<void> > fs(nthreads);
  const auto t0 = gettime();
  auto njobs_left = njobs;
  const auto njobs1 = (njobs + nthreads - 1) / nthreads;
...
void run_serial(double time, ptrdiff_t njobs, const vector<int> &locs,
                ptrdiff_t nthreads) {
...
void run_daisychain_func(double time, ptrdiff_t njobs, ptrdiff_t nthreads) {
...
void run_parallel_func(double time, ptrdiff_t njobs, ptrdiff_t nthreads) {
...
void run_parallel(double time, ptrdiff_t njobs, const vector<int> &locs,
                  ptrdiff_t nthreads) {
...
void run_sqrt(double time, ptrdiff_t njobs, const vector<int> &locs,
              ptrdiff_t nthreads) {
...
void rpcbench_local_functions() {
  ...
  for (auto time : times) {
    cout << "  Benchmark: busywait (" << time * 1.0e+6 << " µsec)" << endl;
...
void rpcbench_global_actions() {
  ...
  for (int dest = 0; dest < rpc::server->size(); ++dest)
    locs[dest] = dest;
