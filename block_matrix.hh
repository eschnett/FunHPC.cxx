...
public:
  ...
  operator std::string() const { return mkstr(*this); }
  std::ptrdiff_t num_blocks() const { return str->num_blocks(); }
  std::ptrdiff_t block_begin(std::ptrdiff_t b) const {
...
public:
  ...
  const rpc::client<matrix_t> &block(std::ptrdiff_t ib,
                                     std::ptrdiff_t jb) const {
  ...
  auto fgemv(bool trans, double alpha, const block_vector_t::const_ptr &x,
             double beta,
