...
std::enable_if_t<cxx::is_function<C<T> >::value, C<T> > munit(T1 &&x) {
  return C<T>([x](const typename C<T>::argument_type &) { return x; });
}

template <template <typename> class C, typename T, typename A>
std::enable_if_t<cxx::is_function<C<T> >::value, C<T> > mmake(A &&x) {
...
std::enable_if_t<std::is_same<C2<T>, C<T> >::value, C<T> >
