...
token parallel(token tok, std::int64_t items, std::int64_t iters) {
  std::vector<hpx::future<token>> fs(iters);
  for (auto &f : fs)
    f = hpx::async(do_work, tok, items);
  for (auto &f : fs)
    tok += f.get();
  return tok;
}
...
token daisychained(token tok, std::int64_t items, std::int64_t iters) {
  ...
  --iters;
  tok = do_work(tok, items);
  auto f = hpx::async(daisychained, tok, items, iters);
  return f.get();
}
...
token tree(token tok, std::int64_t items, std::int64_t iters) {
  ...
  auto f1 = hpx::async(tree, tok, items, iters1);
  auto f2 = hpx::async(tree, tok, items, iters2);
