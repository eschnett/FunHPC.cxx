...
void hwloc_run(bool do_set, int nthreads, atomic_flag *worker_done,
               vector<string> *infos) {
  ostringstream os;
  const int thread = rpc::this_thread::get_worker_id();
  assert(thread >= 0 && thread < nthreads);
  if (worker_done[thread].test_and_set()) {
...
string hwloc_run_on_threads(bool do_set) {
  ...
  for (int attempt = 1; attempt <= nattempts; ++attempt) {
    ...
    for (int thread = 0; thread < nthreads; ++thread)
      worker_done[thread].clear();
    vector<string> infos(nthreads);
    for (int submit = 0; submit < nsubmit; ++submit) {
      fs.push_back(
          rpc::async(hwloc_run, do_set, nthreads, worker_done, &infos));
    }
    for (auto &f : fs)
      f.wait();
    delete[] worker_done;
