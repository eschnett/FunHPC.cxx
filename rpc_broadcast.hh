...
struct reduce_impl {
  ...
  Z z;
  client<C> c;
  reduce_impl(const R &r, const Z &z, const client<C> &c) : r(r), z(z), c(c) {
    RPC_ASSERT(c.is_local());
  }
...
auto reduce1(const R &r, const client<C> &c, const I &b, const I &e)
...
auto reduce(const R &r, const Z &z, const client<C> &c, const I &b, const I &e)
    -> typename C::value_type {
  typedef typename C::value_type A;
  typedef A B;
