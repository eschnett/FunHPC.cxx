...
/*
  ...
  (created by the default constructor), a unit constructor (from a
...
/*
  ...
  C++ requires that at least some of these be defined outside the
  class.)
...
/*
  ...
  thread. There will be no parallelism inside a grid -- to make things
  ...
  For example, one could optimistically make ten copies of a grid at
  ...
  general, it makes sense to define functions for these four traversal
  ...
  would make sense. The lambda is a callback that is called from the
...
/*
  ...
  evaluated at some point in the future -- and without waiting for
  ...
  call to norm in async to make it asynchronous, while in most cases,
...
/*
  ...
  have a future of some object (e.g. the state s0), and want to
  ...
  must make sure that only one thread at a time accesses a stream.
  ...
  To make this work, it is necessary that the output streams are never
...
/*
  ...
  Depending on how threads are scheduled, some threads may still be
